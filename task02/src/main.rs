...
fn collide_particle_ball(
    ...
    // use relative velocity, assume ball is static
    let relative_v = (p.velo - *ball_velo).dot(&plane_norm);
    // solution of impulse conservation and coefficient of restitution quation.
    // notice impulse has direction
    let delta_vp = (2f32 * ball_mass * relative_v) / (particle_mass + ball_mass);
    let impluse = particle_mass * (delta_vp);

    *ball_velo += (impluse / ball_mass) * plane_norm;
    p.velo -= delta_vp * plane_norm;
    // no edit from here
    ////////////////////////
}
